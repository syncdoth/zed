...
async def test_proxy(base_url: str = "http://localhost:8080"):
    ...
    async with httpx.AsyncClient() as client:
        ...
        for i, test_case in enumerate(TEST_REQUESTS, 1):
            ...
            try:
                response = await client.post(f"{base_url}/predict_edits/v2", json=test_case["request"], timeout=30.0)
                ...
                if response.status_code == 200:
                    ...
                    accept_response = await client.post(
                        f"{base_url}/predict_edits/accept", json={"request_id": result["request_id"]}
                    )
                    if accept_response.status_code == 200:
                        print("✅ Accept endpoint working")
                    else:
